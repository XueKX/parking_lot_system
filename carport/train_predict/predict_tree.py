...
path_yuce_dir ="C:/Users/wangquan/Desktop/tree/test_result/"
...
def predicts_tree(names_not_inuse,dataframe_list,path_model_dir,path_log,path_yuce_dir):
    ...
    for i in range(dataframe_num):
        ...
        if chepai_name in names_not_inuse:
            ...
            for j in range(24):
                ...
                yuce = predict_tree(model,x_yuce)
                yuce_value_list.append(yuce)
            data_pre = pd.DataFrame({"车牌号":chepai_name,"未来24小时车位闲置概率":yuce_value_list})
            data_save_path = path_yuce_dir + chepai_name + "_tree_pre.csv"
            data_pre.to_csv(data_save_path, index=False, encoding='utf_8_sig')

if __name__ == "__main__":
    predicts_tree(names_not_inuse, dataframe_list, path_model_dir, path_log, path_yuce_dir)
